    ...
    <>
      ...
      <nav className="fixed top-0 w-full flex items-center justify-between px-6 py-4 border-b border-[#222] bg-black/80 backdrop-blur-md z-50">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-400">
          ...
          <Link href="/blog" className="hover:text-[#FFD400] transition-colors">{tNav('blog')}</Link>
          <Link href="/recharge" className="text-[#cca900] hover:text-[#FFD400] transition-colors font-bold">{tNav('recharge')}</Link>
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-2 w-32 bg-[#111] border border-[#333] rounded-md overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col">
              {locales.map(l => (
                <Link key={l.code} href="/" locale={l.code as any} className="px-4 py-2 hover:bg-[#222] text-white text-left transition-colors">
                  {l.name}
                </Link>
              ))}
            </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-[73px] bg-black z-40 flex flex-col p-6 overflow-y-auto animate-in fade-in duration-200">
          <div className="flex flex-col gap-6 text-lg font-medium">
            ...
            <Link href="/blog" onClick={() => setIsMobileMenuOpen(false)} className="border-b border-[#222] pb-4">{tNav('blog')}</Link>
            <Link href="/recharge" onClick={() => setIsMobileMenuOpen(false)} className="text-[#cca900] font-bold border-b border-[#222] pb-4">{tNav('recharge')}</Link>
            
            <div className="border-b border-[#222] pb-4">
              <p className="text-sm text-gray-500 mb-4">Select Language</p>
              <div className="grid grid-cols-2 gap-4">
                {locales.map(l => (
                  <Link key={l.code} href="/" locale={l.code as any} onClick={() => setIsMobileMenuOpen(false)} className={`px-4 py-3 rounded-lg text-sm ${locale === l.code ? 'bg-[#cca900] text-black font-bold' : 'bg-[#111] text-white'}`}>
                    {l.name}
                  </Link>
                ))}
              </div>
